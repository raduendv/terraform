...
func TestLocal_planOutputsChanged(t *testing.T) {
	b, localBackend := TestLocal(t)
	testStateFile(t, localBackend.StatePath, states.BuildState(func(ss *states.SyncState) {
	...
	cfg := cty.ObjectVal(map[string]cty.Value{
		"path": cty.StringVal(localBackend.StatePath),
	})
...
func TestLocal_planTainted(t *testing.T) {
	b, localBackend := TestLocal(t)
	p := TestLocalProvider(t, b, "test", planFixtureSchema())
	testStateFile(t, localBackend.StatePath, testPlanState_tainted())
	outDir := t.TempDir()
	planPath := filepath.Join(outDir, "plan.tfplan")
	op, configCleanup, done := testOperationPlan(t, "./testdata/plan")
	defer configCleanup()
...
func TestLocal_planRefreshFalse(t *testing.T) {
	...
	p := TestLocalProvider(t, b, "test", planFixtureSchema())
	testStateFile(t, localBackend.StatePath, testPlanState())
...
func TestLocal_planDestroy_withDataSources(t *testing.T) {
	...
	TestLocalProvider(t, b, "test", planFixtureSchema())
	testStateFile(t, localBackend.StatePath, testPlanState_withDataSource())
